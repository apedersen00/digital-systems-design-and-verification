...
To create the burst read/write testbench we define a class, \texttt{mem\_transaction}, that contains constrained parameters for a single transaction. The values can be instantiated and used in the testbench. The memory module was then instantiated in a SystemVerilog testbench that repeats randomized burst transactions 20 times. For each iteration, the randomized transaction parameters are printed to the console. The \texttt{mem\_transaction} class is shown on the next page, but the relevant files can be found in:

\begin{itemize}
    \item \texttt{homework\_4/hdl/q4/sim/tb\_mem\_64kib\_constraint.sv}
    \item \texttt{homework\_4/hdl/q4/sim/mem\_transaction.sv}
\end{itemize}

\begin{svcode}{mem\_transaction.sv}
...
Below the console output of the constrained testbench is shown. The waveform of the first three transactions are shown in \cref{fig:q4_wave}.

\begin{textcode}{q4 - Constrained testbench output}
Transaction 0 ( READ): burst_len=2 addr=0x000059bc write_en=0000
Transaction 1 (WRITE): burst_len=2 addr=0x100010d4 write_en=0100
Transaction 2 ( READ): burst_len=5 addr=0x00004a4c write_en=0000
Transaction 3 ( READ): burst_len=6 addr=0x00007ae8 write_en=0000
Transaction 4 (WRITE): burst_len=2 addr=0x1000e9bc write_en=0010
Transaction 5 (WRITE): burst_len=5 addr=0x100082d4 write_en=0001
Transaction 6 ( READ): burst_len=4 addr=0x0000cf58 write_en=0000
Transaction 7 ( READ): burst_len=5 addr=0x0000bcc8 write_en=0000
Transaction 8 (WRITE): burst_len=2 addr=0x1000172c write_en=1000
Transaction 9 ( READ): burst_len=8 addr=0x00009568 write_en=0000
Transaction 10 ( READ): burst_len=2 addr=0x00004a24 write_en=0000
Transaction 11 (WRITE): burst_len=6 addr=0x1000562c write_en=0100
Transaction 12 ( READ): burst_len=2 addr=0x00002174 write_en=0000
Transaction 13 (WRITE): burst_len=8 addr=0x100036f8 write_en=1111
Transaction 14 (WRITE): burst_len=6 addr=0x100099c8 write_en=0001
Transaction 15 ( READ): burst_len=6 addr=0x0000427c write_en=0000
Transaction 16 ( READ): burst_len=4 addr=0x0000ce80 write_en=0000
Transaction 17 (WRITE): burst_len=2 addr=0x100060e8 write_en=0100
Transaction 18 ( READ): burst_len=6 addr=0x00006330 write_en=0000
Transaction 19 ( READ): burst_len=3 addr=0x00000cf8 write_en=0000
\end{textcode}

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{assets/q4_wave.pdf}
    \caption{Waveform for constrained memory testbench.}
    \label{fig:q4_wave}
\end{figure}
...
\begin{itemize}
    \item Whenever the output $w$ is high, the last four input bits equal 1011.
    \item The output $w$ is asserted for only one clock cycle per detection.
    \item Every property you write must include a reset condition using disable \texttt{iff(!rst\_n)}.
\end{itemize}

\subsection*{Solution}

To implement this assertion based testbench, we start by creating simple stimulus for the sequence detector. The stimulus consists of the 1011 sequence followed by 10 random bits:

\begin{svcode}{q5 - Stimulus for Moore sequenc detector}
  initial begin
    rst_n = 0;
    j     = 0;
...
To correctly verify that the FSM triggers on the correct sequence, the last four input bits are stored in a logic vector with the use of an \texttt{always\_ff} block:

\begin{svcode}{q5 - Save last four input bits to sequence detector}
  logic [3:0] last_4;
...
We can now simply define the two properties and their corresponding assertions:

\begin{svcode}{q5 - Properties and assertions for verification of sequence detector}
...
Luckily, the testbench passed without any assertions. For sanity the waveform is included and seen in \cref{fig:q5_wave}.

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{assets/q5_wave.pdf}
    \caption{Waveform for Moore 1011 testbench.}
    \label{fig:q5_wave}
\end{figure}
