...
#include <random>
#include <verilated.h>
#include "Vtb_mem_64kib.h"

void tick(const std::unique_ptr<VerilatedContext>& contextp, const std::unique_ptr<Vtb_mem_64kib>& top) {
    contextp->timeInc(5);
    top->clk = 0;
    top->eval();
...
void wait_for_ready(const std::unique_ptr<VerilatedContext>& contextp, const std::unique_ptr<Vtb_mem_64kib>& top) {
    while (!top->ready) {
        tick(contextp, top);
    }
}
...
int main(int argc, char** argv) {
    ...
    const std::unique_ptr<Vtb_mem_64kib> top{new Vtb_mem_64kib{contextp.get(), "TOP"}};
